                ...
                if req_field not in metadata or not metadata[req_field]:
                    self.result.issues.append(
                        ValidationIssue(
                            ...
                            suggestion=f"在 frontmatter 中添加 {req_field} 字段",
                        )
                    )

            # 检查推荐字段
            for rec_field in self.RECOMMENDED_FIELDS:
                if rec_field not in metadata or not metadata[rec_field]:
                    self.result.issues.append(
                        ValidationIssue(
                            level="warning",
                            category="metadata",
                            file=str(journal_file),
                            message=f"缺少推荐字段: {rec_field}",
                            suggestion=f"考虑添加 {rec_field} 以完善元数据",
                        )
                    )
                        ...
                        if rel_path.name not in content and str(rel_path) not in content:
                            self.result.issues.append(
                                ValidationIssue(
                                    ...
                                    suggestion="运行 write_journal 重新生成索引",
                                    auto_fixable=True,
                                )
                            )
