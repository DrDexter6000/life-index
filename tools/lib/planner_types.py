#!/usr/bin/env python3
"""Shared planner record dataclasses — L2 deterministic types.

Extracted from ``tools.smart_search.planner`` to eliminate the L2→L3 upward
dependency in ``tools.search_journals.orchestrator`` (PAS-4 layer-invariant
hardening).  All three symbols (StageRecord, PlannerRecord, builder helpers)
are purely deterministic data structures with zero LLM / L3 coupling.

``tools.smart_search.planner`` re-exports from this module for backward
compatibility so existing consumers (tests, L3 callers) are unaffected.
"""
