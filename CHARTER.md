...
> **修订次数**：4
> **最近修订**：2026-05-14 — §1.10 模块-基础层契约边界（Round 20 RFC；记录的三方评审覆盖 §5.2 24h cooldown，详见 RFC 元数据 `cooldown: waived_by_three_party_review`）
...
5. **基元升格回流流程**：当**多个模块**在生产路径上重复需要同一项**确定性、跨模块复用、低 LLM 含量、对 50 年数据语义有跨时间稳定性**的基元时，或当开发者/用户**明确判定**某项基元具备跨模块长期价值时，必须通过 RFC 流程将其升格为 L2 CLI 原语。不满足「确定性 ∧ 低 LLM ∧ 长期语义稳定」且未获明确 developer/user determination → 留在模块本地。CLI 的扩张必须以 RFC 与 cooldown 为代价；模块的扩张以模块自己为代价。
...
**三方评审覆盖机制**：
在特殊情况下，24 小时冷静期可通过**记录的三方评审**覆盖：
- 主审（lead reviewer）提出覆盖建议
- 副审（secondary reviewer 或 read-only advisory reviewer）附议
- 作者/用户做出最终决定
- 覆盖必须在 RFC 元数据中显式记录（`cooldown: waived_by_three_party_review`），并在 `docs/charter-history/` 中归档说明覆盖理由

此机制不得用于削弱 §5.3 所列不可修订章节。覆盖记录本身是审计线索，不是惯例。
