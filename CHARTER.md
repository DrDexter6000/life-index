...
Life Index 是 **agent ecosystem 的 CLI 基元层**，不是独立的 LLM 应用。高级功能可以智能、长程、慢速、需要多轮检索与综合，但其默认形态必须是：Life Index 提供可审计的证据、索引、测算、骨架与提示脚手架，由 calling agent 负责语言合成与解释。

**核心规则**：
- L2 / L3 模块的**默认执行路径**不得持有、配置、或调用任何 LLM
- 模块默认应输出确定性数据或可审计脚手架（如 search 结果、entity graph、aggregate/analyze 结果、evidence pack、claim envelope、scaffolded prompt、agent instructions）
- 改写、合成、解释、对话、翻译、摘要、情感判断等"语言工作"默认由 calling agent 使用其自身 LLM 完成

**明确禁止**：
- ❌ 模块默认路径隐含 bundled LLM、provider client、API key 读取或外部 LLM 请求
...
**明确允许**：
- ✅ 为没有 calling agent 的终端用户提供显式 provider opt-in fallback（例如显式指定 provider 参数），并在执行前清楚说明 provider 与数据暴露范围
- ✅ 模块默认输出 `evidence_pack`、`claim_envelope`、`scaffolded_prompt`、`agent_instructions` 等，交由 calling agent 合成
- ✅ 模块内部使用确定性算法、规则引擎、模板填充、FTS、向量检索、聚合测算、entity graph 与索引树导航

**判断方法**：问"如果用户没有配置任何 LLM provider，这个模块是否仍能完成其核心价值？"

- 能输出确定性结果、证据包、候选集、骨架或可执行提示 → 合宪
- 只能在模块自己调用 LLM 后才有核心价值 → 违宪

**与 §1.5 的关系**：
- §1.5 划定 "L2 不得调 LLM" 的层级边界
- §1.9 划定 "模块默认不得拥有 LLM" 的架构边界
- 二者互补：§1.5 防层级泄漏，§1.9 防每个高级模块各自捆绑 LLM

**违宪示例**：
- ❌ `life-index write` 默认 outbound 调用 LLM 做元数据抽取
- ❌ `life-index smart-search` 默认初始化 OpenAI-compatible client 并合成答案
- ❌ Memoir Engine / Digital Soul / 虚拟家属对话模块各自要求独立 API key 并直接生成最终叙事

**合宪示例**：
- ✅ `life-index write` 默认只写 Markdown + deterministic frontmatter；LLM 辅助抽取只能显式 opt-in
- ✅ `life-index smart-search` 默认输出 evidence pack 与 scaffolded prompt；calling agent 负责最终答案
- ✅ Memoir Engine 默认输出章节骨架、证据索引、引用边界与写作提示；calling agent 负责叙事文字
...
3. **Semantic / 向量检索作为 explicit opt-in**：
   - L2 默认 retrieval 路径不调用向量检索（embedding noise 与 recall-first 承诺有结构性张力）
   - 向量检索代码保留，仅在以下情形启用：
     a. 用户显式 `--semantic` flag
     b. L3 agent 编排显式请求
     c. Zero-result keyword fallback（已 shipped 的 ADR-006 行为，保留）
   - 向量检索的 noise 必须由 calling agent (L3) 或用户显式 acknowledgement 负责过滤，不得污染 L2 默认 result set
...
**与其他条款的关系**：
- 与 §1.5：§1.5 划定"L2 不得调 LLM"层级边界；§1.11 进一步划定"L2 默认 retrieval 不得调向量检索作为主路径"产品承诺边界。二者互补：§1.5 防 LLM 渗透，§1.11 防"概率性 retrieval"渗透 L2 真相承诺。
- 与 §1.6（向下兼容）：本条不修改任何 L1 数据格式；只规约 L2 runtime 默认行为。`.index/` 重建可继续生成 vector index，但默认不启用其结果路径。
- 与 §1.7（三条底线）：reinforced —— L2 默认 keyword-only 简单可审计；向量阈值/noise gate 调参是自动化陷阱；keyword retrieval 比 hybrid 更快且更 deterministic。
...
- 与 §3.2（双管道）：见 §3.2 amendment note（双管道作为确定性原语保留，默认形态收紧为 keyword-only）。
- 与 §4.1（架构违宪）："在 retrieval / ranking 层硬切 top-K" 已是违宪；§1.11 强化此条并扩展到"默认开向量检索作为主路径"。

**§5.3 保护**：本条 §1.11 加入 §5.3 不可弱化清单 —— 只允许变得更严，不允许变得更松。未来"为让 paraphrase 数字好看而打开 hybrid default"的提案，必须先通过 §5.2 修订本条。

**触发场景与起源**：v1.2.0 cycle2 absorption 跑出 eval baseline 后，CTO 与主理人 surface 两个长期 loophole：(1) semantic 阈值 dilemma（高漏低噪）；(2) hard truncation 20 与"不遗漏"承诺冲突。Root cause = truthfulness model 从未定调，导致 eval gate / production runtime / dataclass default 三处 default 不一致。本条 §1.11 enshrine 产品承诺为宪法不变量，统一 default 方向。详见 RFC-2026-05-23-l2-recall-first-truthfulness-model 与 ADR-028。
...
> **§3.2 amendment（v1.6.0, 2026-05-23, RFC-2026-05-23-l2-recall-first-truthfulness-model）**：本条 §3.2 提及的"双管道"指其作为确定性搜索原语的存在（FTS5 + 向量 + RRF 均保留可用、保留 deterministic 性质、保留不调 LLM 性质）；其 **默认 runtime 启用形态** 见 §1.11。即 default search path = keyword pipeline only；向量 pipeline 通过 explicit opt-in（`--semantic` flag / L3 agent 显式请求 / zero-result fallback per ADR-006）启用。§3.2 主体的"地基不可替换"承诺保持不变 —— 双管道仍是地基，本 amendment 仅规约其默认形态。
...
**宪章级约束**：
- 编排器**调用**双管道和其他 L2 工具，**不得修改**这些确定性工具的内部实现
- 编排器暴露为独立命令（如 `life-index smart-search`）或独立模块
- `life-index search` 保持纯双管道行为，永不加载 LLM
- 编排器必须遵守 §1.9：默认路径不得持有、配置、或调用 LLM；如提供 LLM synthesis，只能走显式 opt-in fallback
