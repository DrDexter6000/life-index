...
def _dated_target_dir(target_dir: Path) -> Path:
    now = datetime.now()
    return target_dir / str(now.year) / f"{now.month:02d}"
...
async def download_url(
    ...
    *,
    ...
    dated_dir = _dated_target_dir(target_dir)
    ...
    *,
    ...
    async def _download(url: str) -> dict[str, Any]:
        async with semaphore:
            return await download_url(url, target_dir)
