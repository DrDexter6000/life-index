...
- [ ] §1.9 增量解释条款写入 `CHARTER.md`；closeout 归档 `docs/charter-history/`（实现/closeout 阶段）
- [x] RFC status `proposed` → `accepted`

**实现验收（由首个消费者 GUI 拉动，post-ack；派发时机按 Freeze §8 主理人拍板）**：

- [ ] `tools/agent_bridge/`（L3）：确定性解析 + OpenAI-compatible 传输（+ ACP 薄适配）
- [ ] `brain` 配置 schema（§3.6）落地，含 `data_exposure_ack` 强制
- [ ] Onboard 能力检测 SOP（§3.7），含探针 round-trip
- [ ] `test_layer_invariants.py` 通过（L2 无 `agent_bridge` / LLM client import）
- [ ] **P1 真机探针**：GUI → 本机 Hermes `:8642` → 消费 smart-search scaffold → 合成 →
      CLI validate/apply，端到端通过
- [ ] **P2 探针**：对一个 OpenAI-compatible provider 通过
- [ ] **降级探针**：清空 brain 配置，deterministic-only 仍输出 evidence/scaffold
