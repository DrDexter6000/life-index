...
> **在文档体系中的角色**：本文档现在主要作为 `AGENT_ONBOARDING.md` 与 `AGENT_ONBOARDING_WEB.md` 在 Step 0 自动分流后的**升级 / 修复权威说明**。普通用户通常不需要先自己判断是否该读本文档；Agent 应先刷新 authority documents，再同步 canonical checkout，之后才切换到这里执行。

> **canonical checkout 原则**：如果宿主 Agent 平台已经管理了 canonical skill checkout，应优先复用它。不要在 generic workspace 根目录再创建第二份 `life-index`，除非用户明确要求不同位置。
...
### Step 1 — 刷新 authority documents

在执行任何升级动作前，先刷新 `bootstrap-manifest.json`，并把它视为版本/authority anchor。

然后按 manifest 的 `required_authority_docs` 刷新这些权威文件：
...
### Step 3 — 同步 canonical checkout（强制）

如果发现多个 Life Index checkout，先确定 canonical checkout，再继续执行升级。默认优先：

1. 宿主 Agent 平台已管理的 canonical checkout
2. 用户明确指定的位置
3. 其他已有且能通过 health 验证的 checkout

如果仍然无法判断，不要自动创建第三份 checkout，先向用户汇报冲突。

如果网络可用，agent **必须**先同步 canonical checkout，再继续升级。
...
- `bootstrap-manifest.json` 存在
- `life-index --version` 可返回 package version 与 bootstrap manifest 信息
- 当前 checkout 不是未同步的旧缓存

如果同步失败：

- 不要把现有 checkout 继续当成“已经最新”
- 转入 repair 流程，必要时向用户报告 checkout 异常
...
如果升级流程结束后用户希望“立刻可用”，Agent 还应明确说明：
...
1. authority documents 已刷新
2. canonical checkout 已同步
3. `life-index --version` 可返回 package version 与 bootstrap manifest 信息
4. `life-index health` 成功且未报告不健康状态
5. 对现有数据的一次已知搜索返回预期结果
6. 如果 release notes 提到检索/索引/搜索变更，已运行 `life-index index`
7. 如果 health 或搜索仍有异常，已考虑或执行重建流程
