...
> **本文档职责**: 详细工作流步骤参考（供完整流程查阅）
> **快速参考（触发词/工具速查/关键约束）** → 详见 [SKILL.md](../SKILL.md)
> **适用对象**: 具备文件操作能力的 AI Agent（Claude、OpenClaw 等）
> **版本/状态**: 详见 [README.md](../README.md)
>
> **相关文档**: [快速参考→SKILL.md](../SKILL.md) | [架构→HANDBOOK.md](./HANDBOOK.md) | [定时任务→SCHEDULE.md](../references/schedule/SCHEDULE.md) | [变更历史→CHANGELOG.md](./CHANGELOG.md)
---
...
**触发词、工具速查、核心约束** → 详见 [SKILL.md](../SKILL.md)

本文档提供详细工作流步骤，供需要完整流程参考时使用。

---

## 分类体系

Topic分类、Project命名、Tags规范详见 [API.md](./API.md#topic-分类定义)。

---

## 工具接口

工具详细参数和返回值见 [API.md](./API.md)（参数/返回值/错误码以 API.md 为准）。
...
**调用示例**:
```bash
python -m tools.write_journal --data '{"title":"...","content":"...","date":"2026-03-07","topic":["work"],"abstract":"...","mood":[],"people":[],"project":"","tags":[],"links":[]}'
```
...
**调用示例**:
```bash
python -m tools.search_journals --topic work --project Life-Index --date-from 2026-01-01 --limit 10
```
...
**调用示例**:
```bash
python -m tools.edit_journal --journal "Journals/2026/03/life-index_2026-03-07_001.md" --append-content "补充内容"
```
...
**元数据字段定义、必填规则、Agent职责** → 详见 [SKILL.md#required-metadata-fields](../SKILL.md#required-metadata-fields)

**快速要点**：
- 所有字段必须包含（即使为空值）
- title, content, date, abstract, topic, mood, tags 必须有值
- 从内容中提取 mood(1-3个)、tags、people、project

#### 天气处理三层机制

**详细流程** → 详见 [WEATHER_FLOW.md](../references/WEATHER_FLOW.md)  
**关键约束（强制确认）** → 详见 [SKILL.md#天气与地点确认](../SKILL.md#天气与地点确认)

| 层级 | 场景 | 处理方式 |
|:----:|------|---------|
| 1 | 用户提供地点+天气 | 直接使用 |
| 2 | 用户未提供 | 自动填充默认地点 + 查询天气 |
| 3 | 写入后 | **必须**检查 `needs_confirmation`，展示并确认 |
| 4 | API 失败 | Agent 使用网络搜索 Fallback |
