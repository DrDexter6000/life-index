...
| 视觉风格 | Celestial Editorial 暗色主题，深空背景 + 琥珀金强调 | 品牌识别 |
| 启停方式 | `life-index serve [--port PORT]`，Ctrl+C 关闭 | 符合"按需运行、一键关闭"原则 |

### 2.1 视觉设计规范（Celestial Editorial）

基于 `docs/web-gui/UIUX/DESIGN-DIRECTION.md` 的 Celestial Editorial 美学体系，Web GUI 采用以下视觉参数：

**色板**
- 背景：`#0A0E14`（深空青黑）
- 表面层：`#12161E` / `#1A1E28` / `#1E2330`
- 强调色：琥珀金 `#FFE792`、珊瑚 `#F9873E`、青绿 `#85FFF2`
- 文字：主文字 `#F9F9FF`、次要文字 `#C4C6CF`

**圆角规范**
- 大视觉元素（卡片、容器）：`rounded-2xl` (1.5-2rem)
- 小视觉元素（按钮、输入框）：`rounded-xl` (0.75rem)
- 禁止 `rounded-full` pill 形状

**导航栏（Header）**
- 背景透明度：`rgba(18, 22, 30, 0.15)`（高透明）
- 模糊效果：`blur(24px)`
- 阴影：`0 20px 40px rgba(0, 0, 0, 0.4)`

**背景动态效果**
- 多层径向渐变光晕（琥珀、珊瑚、青绿、紫色）
- 动画：`ambient-drift` 12s 循环
- 缩放范围：1.0 → 1.32（±32%）
- 位移范围：±8%
- 颗粒纹理：静态，`opacity: 0.025`，`baseFrequency: 1.2`
