...
```
完成时间: 2026-05-21
实际 LOC: ~936 lines added (tools/recall/ ~240, tests ~450, docs ~250)
遇到的难点:
  1. recall mode hybrid search may return semantic matches for "no_results" tests — adjusted assertion
  2. Windows .pytest_tmp lock file issue with --basetemp config — tests pass with default basetemp
  3. mypy no-any-return on dict access — fixed with list() cast
验收证据:
  - test_recall_contract.py: 12 passed in 258.66s
  - test_layer_invariants.py: 7 passed in 0.52s (incl. 3 new recall invariants)
  - sample recall --mode=default output: {"success":true,"mode":"default","effective_mode":"default","source_command":"search --no-semantic",...}
  - sample recall --mode=recall output: {"success":true,"mode":"recall","effective_mode":"recall","source_command":"search",...}
  - deep degrade test trace: "recall: deep mode requires --use-llm; degraded to recall mode (no LLM)."
  - local commit: 7fb3941 on claude/gbrain-E (not pushed)
```
