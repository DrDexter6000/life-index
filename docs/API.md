...
| Gold Set Recall@5 | ≥ 最新冻结基线 | 使用本地 / 私有 eval baseline（若存在） |
| Gold Set P@5 | ≥ 最新冻结基线 | 使用本地 / 私有 eval baseline（若存在） |
| Gold Set MRR@5 | ≥ 最新冻结基线 | 使用本地 / 私有 eval baseline（若存在） |

> Round 17 冻结基线（keyword-only, 85 queries）：Recall@5=0.3836 / P@5=0.3565 / MRR@5=0.2716 仅作为**历史地板**参考。当前回归门控应以本地 / 私有冻结 baseline 为准；公开 checkout 不携带该数据时，数据依赖型 eval 测试显式 skip，详见 CHARTER §4.5。
...
| `answer` / `answer.*` | 当前 `--synthesize` 的 provider-backed LLM synthesis + trust gate 输出；see the named `--synthesize` transition authority block | 优先展示当前 answer output | **stable** |
...
| `--synthesize` | 内部构建 evidence；当前添加 provider-backed LLM synthesis + trust gate answer；see the named `--synthesize` transition authority block |
| `--include-evidence --synthesize` | 添加 evidence_pack + 当前 provider-backed LLM synthesis + trust gate answer；see the named `--synthesize` transition authority block |
...
`life-index eval` uses a local/private `golden_queries.yaml` as the search
quality Gold Set authority when that file is present. Public source releases
ship the eval harness but not the data. When no local query set is present,
data-dependent eval runs return an explicit skipped state instead of inventing
placeholder results. The same file may also contain companion sections for
...
公开源码只包含 eval harness。数据依赖文件按以下约定留在本地：

1. `LIFE_INDEX_EVAL_DATA_DIR` 指向的目录。
2. 若未设置该变量，则使用当前 `LIFE_INDEX_DATA_DIR/.eval/`。
...
当这些文件缺失时，数据依赖 eval 返回显式 skipped 状态；提供本地数据后，同一
harness 会运行完整检查。
