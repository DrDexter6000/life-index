...
| `.github/workflows/quality.yml` | push to main/develop and pull requests to main | Tier 1 | doc sync, public diff name scan, lint, format, security, and type checks |
...
| 6 | lint flake8 | `flake8 tools/ --count --max-complexity=40 --max-line-length=100 --show-source --statistics` | quality.yml | same |
| 7 | format black | `black --check --diff tools/` | quality.yml | `python -m black --check tools/` |
| 8 | security bandit | `bandit -r tools/ -ll -c pyproject.toml` | quality.yml | same |
| 9 | typecheck mypy | `mypy tools/ --ignore-missing-imports` | quality.yml | same |
