...
> **2026-05-23 §1.11 enshrine 后的对齐说明**：CHARTER v1.6.0 §1.11 把 "L2 default keyword-only, semantic explicit opt-in" 升格为 50 年宪法不变量。本节描述的 "关键词检索是 eval gate 官方口径" 与 "user-facing CLI 使用 zero-result fallback" 模式从此具有 CHARTER 不变量背书。任何代码层 default 与本节描述不一致（包括 `tools/search_journals/core.py:1026` 默认 mode、`tools/search_journals/query_types.py:89` dataclass default）必须按 §1.11 对齐到 keyword-only default。详见 RFC-2026-05-23 / ADR-028。
...
| **S2 智能搜索编排** | L3 | `life-index smart-search` 输出 agent-ready 确定性检索 scaffold；意图识别、query expansion、有界多轮 search 调用、结果精筛/摘要由宿主 agent 与 Skills 完成 | v1 contract，工具确定性 |
...
- LLM 的合法位置是宿主 agent / Skills：进入检索前做搜索编排，检索后做过滤/摘要/解释；`smart-search` 只输出确定性检索 scaffold、`agent_instructions`、`answer_scaffold` 与 `query_plan`。LLM 不得绕过 CLI Core 直接读写 L1 数据。
- 高级模块必须建立在稳定记录格式、结构化检索、Entity Graph 增强、语义补充与宿主 agent 编排之上。
...
**核心原则**：每一层是过滤器，不是数据源。Pipeline A（关键词+结构化检索）内部执行三层递进过滤：L1 导航层快速预筛（Index B facet docs + manifest），L2 元数据层多维度过滤（YAML Frontmatter + SQLite 缓存），L3 FTS5 内容层精确匹配。Pipeline B（语义向量搜索）作为召回补充而非默认路径。User-facing CLI 默认使用 fallback 模式：仅当 Pipeline A 返回零结果时才触发 Pipeline B；`--semantic-policy hybrid` 启用双管道并行执行并经 RRF 融合后返回。
...
- **工具流程**：不初始化 provider client，不读取 LLM key，返回确定性检索 scaffold、`agent_instructions`、`answer_scaffold` 与 `query_plan`；自然语言查询复用 SearchPlan 已抽取关键词作为 bounded 子查询，并显式调用 `search` 的 zero-result semantic fallback；当短关键词被 noise gate 跳过时，`query_plan.semantic_fallback_query` 记录用于 semantic fallback 的原自然语言问题
