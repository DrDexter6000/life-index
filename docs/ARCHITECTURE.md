...
- **工具流程**：不初始化 provider client，不读取 LLM key，返回确定性检索 scaffold、`agent_instructions`、`answer_scaffold` 与 `query_plan`；自然语言查询复用 SearchPlan 已抽取关键词作为 bounded 子查询。关键词选择、二次检索、过滤与总结由宿主 agent 完成
