...
> **实现状态**：当前尚无正式的高级模块（Memoir Engine、心潮地图等仅存在于路线图/远景）。本节作为架构契约占位，供未来模块开发时引用。ADR-026 §3 (Navigable Index Tree) 与 §5 (Addressable intermediate artifacts) 是 §1.10 的具体实现 anchor。面向模块作者的具体消费/状态/升格契约见 [RFC-2026-05-15](./rfc/RFC-2026-05-15-advanced-module-developer-contract.md)。
...
| **S2 智能搜索编排** | L3 | `life-index smart-search` 默认输出 agent-ready 确定性检索 scaffold；`--use-llm` 才启用意图识别、query expansion、有界多轮 search 调用、结果精筛/摘要 | v1 contract，可选增强，可降级 |
...
- LLM 的合法位置是 S2：进入检索前做搜索编排，检索后做过滤/摘要/解释；但 provider-backed 编排必须由 `smart-search --use-llm` 显式启用，默认 smart-search 输出确定性检索 scaffold、`agent_instructions`、`answer_scaffold` 与 `query_plan`。LLM 不得绕过 CLI Core 直接读写 L1 数据。
...
- **默认流程**：不初始化 provider client，不读取 LLM key，返回确定性检索 scaffold、`agent_instructions`、`answer_scaffold` 与 `query_plan`
- **显式 LLM 流程**：仅在 `--use-llm` 下执行前置改写（LLM 拆解 query，最多 3 个 `sub_queries`，并产出 `expanded_terms` / `intent_type` / `time_range`）→ 中间调用（消费 expansion terms、确定性 date filter、intent strategy，逐个调用 keyword-first search 原语并融合去重）→ 后置筛选 + 摘要（LLM 精筛）
