...
> 状态：Revision 2 — Chief Advisor Round 1 adjudicated; pending Human Owner ratification
>
> 日期：2026-07-09
>
...
2. 升格项应与 Core-owned truth 或跨模块稳定基元密切相关，而不是完整领域工作流；
3. RFC 必须包含 falsifier、回滚/降级路径、schema、SLO、错误与回归证据。

在该修订获 Human Owner 批准并进入 `CHARTER.md` 后，本节只保留指针。Whole-feature promotion 不是默认路径；通常只有确定性基元回流 Core。
...
1. **Recall-first 与 eval 同改**：
   ...
   - 同一 work package 重写本地/私有 Gold Set 与 rejection expectation，不能让旧指标继续保卫错误行为；
...
3. **收口具体写入原子性缺口**：保留现有 temp file、atomic rename、pending、Data Doctor 和 `side_effects_status` 资产，重点处理：
   ...
   通过故障注入证明不存在孤儿附件、半更新索引、错误 stale-success 或 committed-but-reported-failed。
...
5. **Transaction**：故障注入证明失败后不存在孤儿附件、半更新索引、虚假 success 或 committed-but-reported-failed。
...
| Tier | 典型范围 | 决策与执行 |
|---|---|---|
| A — Routine / Reversible | 小型文档、卫生、已批准边界内的单文件修复、低风险机械工作 | 主控 Agent 按 `AGENTS.md` 自主执行、聚焦验证、报告证据；无需 CTO/Advisor/Owner 前置审批 |
| B — Scoped Product / Contract Work | 跨模块但仍在已批准战略内的功能、契约实现、复杂 bug、激活改进 | CTO / Product Director 给目标与验收边界；主控 Agent 写具体方案/TDD并执行；Advisor 按风险抽查 |
| C — Strategic / Irreversible | 宪章、公共 API/schema、数据边界、安全、兼容删除、新 Addon、release/tag/push/go-live、重大仓库/发行边界 | 完整 CTO 提案 → Advisor red-team → CTO adjudication → Human Owner 批准 → 主控 Agent执行 → 双方证据复核 |

Tier 由 blast radius、可逆性和公共承诺决定，不由文件数量决定。无法确定 Tier 时按高一级处理，但只读调查永远不因缺少战略审批而禁止。
...
3. **批准 A/B/C 爆炸半径流程**：仅 Tier C 走 CTO + Advisor + Owner 全流程；Routine read-only、小修和已批准边界内实现不被战略流程阻塞。
...
6. **批准 §1.10 升格规则精化方向**：Owner 判定只替代第二消费者证据，不豁免其他条件；正式措辞由后续宪章修订承载，本设计稿不成为并行 SSOT。
...
## 19. Chief Advisor Round 2 Review Questions

Chief Advisor 下一轮应重点挑战：

1. §0.1 对公开 CI 与私有 eval 的事实校正是否准确，还是仍低估了某个真实 blocker？
2. P0 写入风险分类是否完整，尤其是附件、派生索引和 committed-but-reported-failed？
3. §6.2 的“权威先行”是否足以防止 registry 成为第二 SSOT？
4. §7.4 的四类 prompt 所有权是否真正关闭了 bridge 无主知识漏洞？
5. §8.2 是否已消除设计稿与宪章升格规则双轨？
6. A/B/C 流程是否既保护 Tier C，又避免 CTO/Advisor 形成日常瓶颈？
7. P0 的灾难恢复、P1 的 Windows/import 激活与 P3 的 Memoir gate 是否可被客观验收？
8. Human Owner 的六项 ratification 是否还缺少必须上桌的战略分歧？
