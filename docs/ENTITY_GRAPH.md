...
| 6 | `type` 在 `ENTITY_TYPES` 中（`actor`/`artifact`/`place`/`project`/`event`/`concept`） | `entity --check` 验证 |
| 7 | 无 alias 冲突（不与其他实体的 alias/primary_name 重叠） | `entity --check` 验证 |
| 8 | 有明确的添加理由（修复某个查询失败 / 提升 recall / 用户要求） | 文档化在激活报告中 |

### 5.3 批量准入必须两段式

- 不通过旧冷启动写盘入口自动落盘（见 §6.2）
...
- 旧的顶层冷启动写盘 flag 已移除；调用时会返回结构化替代命令错误
...
- `maintain --normalize` 会迁移所有旧 `person` 到 `actor`；普通加载旧 `type: person` 会 fail-closed 并指向 normalize，normalize 专用 loader 负责容忍旧图并生成迁移计划
- 实体 ID 不透明且长期稳定，前缀没有语义；不要从 `person-` / `actor-` 等 ID 前缀推断实体类型
...
| 规则 | 要求 | 说明 |
|------|------|------|
| `type` | `actor` | AI 助手、组织、人类个体都归入 actor，细分写在 `attributes.kind` |
...
**当前实现行为**：删除必须先 preview；apply 必须带 `--backup` 并会创建
`entity_graph.yaml.backup_*`，返回的 `cleaned_refs` 是将被清理或已清理的引用清单。

**执行前必须**：
1. 运行 preview 并让用户确认删除该实体及引用清理影响
2. apply 时保留 `--backup`
3. 删除后运行 `entity --check` 和必要的搜索验证
