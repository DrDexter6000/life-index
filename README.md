...
| | **Life Index** | **PKM**（Notion/Obsidian） | **Agent 记忆**（Mem0/Zep） |
|:---|:---:|:---:|:---:|
| **回答的问题** | "那时的我，什么心情？" | "我知道什么？" | "我刚才说了什么？" |
| **数据组织** | 按时间轴归档 | 按知识图谱 | 按上下文窗口 |
| **数据寿命** | **永久保留** | 长期，但会重构 | 临时，会被遗忘 |
| **服务对象** | **人** | 人 | Agent |

一句话：**Agent 记忆是消耗品，Life Index 是遗产。**
...
在当前常见 LLM/Agent 生态下，单靠自然语言触发技能有时不稳定。  
实测更高成功率的写法是：`/life-index` + 意图词。

```text
/life-index 记日志：今天看到乐乐以前的照片，突然很想念那个2岁的小英雄，那种甜蜜又忧伤的感觉。
/life-index 搜索记录：查找关于“乐乐”的日志。
```
...
把下面整段话直接复制给你的 Agent，然后回车：

```text
请帮我完成 Life Index 终端用户安装与验证（OpenClaw 环境）。

仓库地址：
https://github.com/DrDexter6000/life-index

要求：
1) 读取 README.md 与 SKILL.md 的安装说明
2) 使用终端用户方式安装（优先 pipx install life-index）
3) 将 SKILL.md 放到 ~/.openclaw/workspace/skills/life-index/
4) 执行并汇报验证结果：
   - life-index --help
...
5) 告诉我：
   - life-index 安装路径
   - SKILL.md 实际路径
   - 我下一步记录第一篇日志的推荐命令（使用 /life-index）
```

> 数据目录会自动写入：`~/Documents/Life-Index/`
...
**命令找不到（life-index: command not found）**  
→ 重新执行 `python -m pipx ensurepath`，重开终端后再试

**日志没有写入 ~/Documents/Life-Index/**  
→ 先运行 `life-index --help` 确认 CLI 可用；开发模式请确保在项目根目录运行 `python -m tools.xxx`
