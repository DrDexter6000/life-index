...
三岁之后的乐乐依然是全宇宙最重要最珍贵的存在 ——
但确实，那个让我神魂颠倒的小英雄、她长大了，属于我和那个婴儿的时光、已经一去不复返了。

总而言之，小豆丁，爸爸想你了。

![乐乐2岁的小英雄](attachments/2026/03/2yo_Toto.jpg)

Toto, this one is for you.
```

<p align="center">
  <img src="./assets/2yo_Toto.jpg" alt="2岁的乐乐" width="400">
  <br>
  <em>——那个让我神魂颠倒的小英雄</em>
</p>
...
**CLI Core v1.0.1** 已稳定运行，不是原型，不是 demo——这是一个经过 2,400+ 单元测试、CI 全绿、真实日常使用的系统：
...
<details>
<summary>📚 如何在庞大的人生档案馆中，唤回人生碎片？</summary>

Life Index CLI Core 的分层检索（关键词 + 实体扩展 + 语义补充），可以在**全离线、确定性、零 token 消耗**的情况下，帮助你从未来数十年不断增长的人生日志中，精准而完整地召回关键记忆碎片。
...
但是，很多时候，你记得的不是准确标题、标签或日期，而是一组模糊的线索：

> “过去两个月我是不是经常晚睡？”
>
> “我什么时候开始认真思考 Life Index 这件事？”
>
> “那些后来改变我判断的教训，最早出现在哪里？”

这时，Life Index 暴露给 Agent 的 **CLI 原子接口** 可以接入搜索管道的前后两端：

```
┌─────────────────────────────────────────────┐
│                 Agent 编排层                 │
│  搜索前：意图识别 / 查询改写 / 关键词扩写 / 多轮搜索 │
└───────────────────┬─────────────────────────┘
                    │
                    ▼
┌─────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────┐
│                 Agent 编排层                 │
│  搜索后：结果精筛 / 引用整理 / 摘要 / 报告生成      │
└─────────────────────────────────────────────┘
```

区别在于：Life Index 不是让 AI 直接替你翻完整个人生，而是先用本地结构化检索、实体/关系图谱和语义补充，把证据范围缩小到可靠候选；Agent 只在你需要时，围绕 CLI Core 做编排、筛选和表达。

完整技术架构见 [ARCHITECTURE.md §2](docs/ARCHITECTURE.md)。
...
> *总而言之，小豆丁，爸爸想你了。Toto, this one is for you."*
>
