...
要求：
1. 先刷新并阅读最新 authority files，再开始执行：如果本地已有 checkout，先 `git pull` 刷新，再按 `bootstrap-manifest.json` 的 `required_authority_docs` 逐个刷新并阅读，不要依赖本段文字中的旧列表；如果本地什么都没有（全新机器），直接按 `AGENT_ONBOARDING.md` Step 4.1 克隆，一次全新 clone 即等于把 authority 刷新到最新
2. 在重建 `.venv`、运行 `health`、采纳 checkout、删除任何目录、或判断 fresh install / upgrade / repair 之前，先按 `AGENT_ONBOARDING.md` Step 0 运行 `life-index bootstrap --json` 或 checkout 内 `python -m tools bootstrap --json`；注意：全新机器必须先 clone 再 bootstrap
...
→ 全新 / 空数据安装的 pre-init `degraded` 属预期状态，简要说明即可，不当作失败；若 `safe_next_steps` 包含 `index`，执行后重新运行 health；若 `safe_next_steps` 不包含 `index`，不要自行执行 index
