...
> · **当前实测**：20+ CLI 命令 · 2,400+ unit tests · keyword-only Recall@5 = **0.7857**（cycle2 fixture · 56 queries；约 0.79，相对 0.79 目标为边际 miss；C2 paraphrase 仍为已知 gap）· 2026-05-25 完整 integrity audit PASS
...
**不做云端存储 —— 因为「程序与数据彻底分离」。** 你的人生数据是一份**独立于软件**的纯 Markdown 资产，存在你自己的本地目录里，不进任何人的服务器。Life Index 这个程序可以升级、可以哪天消失，你的数据原地不动、永远可读。换句话说：**程序是租来的，数据是你的。** 这是"数据主权"的工程形态（详见 [设计决策 · 数据主权](#设计决策--design-decisions)）。

**不做工具内 Vector RAG —— 因为实测零增益。** 2026 年几乎所有检索系统都默认"上向量"。我们做了，又主动移除了 —— 因为对个人日志这一场景，它是负 ROI 的过度工程。

| 108-query golden set（2026-06-28） | MRR@5  | Recall@5 | Precision@5 | nDCG@5 |
| -------------------------------- | ------ | -------- | ----------- | ------ |
| 关键词 + Entity Graph               | 0.6259 | 0.9231   | 0.5351      | 0.6602 |
| 关键词 + `--semantic`               | 0.6259 | 0.9231   | 0.5351      | 0.6602 |

**四位小数完全相同，5 个失败查询没有一条被语义救回。** 完整的四层论证（架构 / 检索 / 实测 / 工程）见 → [设计决策 · 为什么不做 in-tool vector RAG](#设计决策--design-decisions)。

---

## 快速开始

CLI、自然语言、GUI 不是三个"版本"，而是三种**交互方式**，服务于不同的人：

| 🖥️ **CLI**       | 🗣️ **自然语言**                                    | 🎨 **GUI**                    |
|:-----------------:|:-----------------------------------------------:|:-----------------------------:|
| *开发者 · 极客 · 集成*   | *有 Agent 平台的用户*                                 | *所有人*                         |
| 用终端直接操作，精确控制每个参数。 | 对 Agent 说"帮我记录今天的心情"，它理解你的意思后调用 CLI。**当前推荐方式。** | 不需要懂技术，用眼睛看、用手指点。*([独立公开仓](https://github.com/DrDexter6000/life-index-gui))* |

### CLI 与 GUI：Agent 与人的双服务关系

**Life Index CLI 服务 Agent**：CLI 是为 Agent 打造的确定性 native 工具层，母语是命令行。它安装在宿主 Agent 环境中，通过显式命令记录和检索日志，没有内建 LLM。

**Life Index GUI 服务人类用户**：GUI 是建于同一 CLI 地基之上的体验层，面向人的书写、阅读、检索与移动使用。它追求艺术独立游戏级的 UI/UX，也追求“走出家门”的移动性：让留在家中主机上的 Agent 与本地数据，经安全隧穿延伸到手机上的随手记录与回忆浏览。

GUI 公开仓：[life-index-gui](https://github.com/DrDexter6000/life-index-gui)。智能仍来自你的宿主 Agent，不来自 GUI；数据仍与程序代码分离。

**不需要 API key，不需要 embedding 模型，不需要云服务** —— CLI Core 默认离线即可跑起来。
...
**CLI Core 当前稳定线（1.3.4）** 已稳定运行 —— 不是原型，不是 demo，是经过 2,400+ 单元测试、CI 全绿、真实日常使用的系统。**地基已成，当前主线是把体验层（GUI v1）立起来。**

<details>
<summary>🧱 已经建好的地基（CLI Core 1.3.4，点击展开）</summary>
...
| 搜索质量评估 (eval)          | ✅   | Cycle 2 多信号 fixture；2026-05-25 完整 integrity audit PASS；recall-first 质量门控        |
...
CLI Core 是地基，GUI 是你看到的建筑。Life Index 的 GUI 体验层正在独立公开仓库 [`life-index-gui`](https://github.com/DrDexter6000/life-index-gui) 中开发 —— 把 **write（+ 智能元数据）** 与 **search（+ smart-search）** 做成不用懂技术也能上手的图形界面。设计语言为「**Soul Shrine · 灵魂神龛**」，融合 Monument Valley 的禅意美学与东方文人气质。

<p align="center">
  <img src="./assets/GUI_prototype.png" alt="GUI 体验层原型预览" width="700">
  <br>
  <em>GUI 体验层原型预览 —— 归宿 (The Core) 界面</em>
</p>

<p align="center">
  <a href="https://raw.githack.com/DrDexter6000/life-index/main/assets/GUI_prototype.html"><strong>🔗 在浏览器中打开交互原型</strong></a>
</p>
...
**③ 实测上 —— 在我们自己的 golden set 上，向量召回是零增益。**
2026-06-28，在 108-query golden set 上对比开 / 关语义检索：
...
**四位小数完全相同，5 个失败查询没有一条被语义救回** —— 语义没带来任何额外命中，只带来低相关的噪声。
