...
1. 向 Agent 说：*"请阅读 SKILL.md，作为我的 Life Index 日志助手。"*
2. 数据目录自动创建：`~/Documents/Life-Index/`
3. 直接开始：*"今天想记录一下..."*
...
cd life-index
pip install -e .                    # 核心安装
pip install -e ".[semantic]"        # 可选：语义搜索（约 2GB 模型）
```

<details>
<summary>📋 命令速查表（点击展开）</summary>

| 操作 | 命令 |
|:---|:---|
| 记录日志 | `python -m tools.write_journal --data '{...}'` |
| 搜索日志 | `python -m tools.search_journals --query "关键词"` |
| 语义搜索 | `python -m tools.search_journals --query "学习笔记" --semantic` |
| 多维过滤 | `python -m tools.search_journals --topic work --people "乐乐"` |
| 编辑日志 | `python -m tools.edit_journal --journal "路径" --set-weather "晴天"` |
| 生成摘要 | `python -m tools.generate_abstract --month 2026-03` |
| 查询天气 | `python -m tools.query_weather --location "Beijing,China"` |
| 构建索引 | `python -m tools.build_index` |

所有工具返回标准 JSON：`{"success": true, "data": {...}, "error": null}`
...
→ 确保使用 `python -m tools.xxx` 而非 `python tools/xxx.py`
