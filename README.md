...
`.index/` 下的 SQLite 检索索引与派生缓存都可以删除重建。你的日志、附件、frontmatter —— 任何文本编辑器都能直接读，不需要 Life Index 在场。**软件可以消亡，数据不会**；您的人生来路，50 年后依然字迹清晰。
...
**关键设计原则**：确定性操作（写日志、按标签/关键词/实体搜索、浏览时间线）由 GUI 直连 CLI Core；需要"思考"的操作（开放式回忆编排、回忆录生成、数字人格）才经由 Intelligence Layer。**能确定的事不问 Agent，需要思考的事才找 Agent。**

**为什么不做 in-tool vector RAG**：Life Index 的 agentic 架构把规划、多跳理解、解释和综合交给宿主 agent，CLI Core 只负责可审计、可复现的 retrieval 原语。2026-06-28 的 108-query golden set 实测显示，关键词 vs 关键词+`--semantic` 四项指标完全相同（MRR@5 0.6259 / Recall@5 0.9231 / Precision@5 0.5351 / nDCG@5 0.6602），5 个失败没有一条被语义救回；因此向量/语义检索不再在工具内运行，`--semantic*` 仅保留为兼容 no-op。
...
| **🔍 search**<br>检索人生 | • FTS5 关键词检索<br>• Entity Graph 扩展<br>• 时间 / 主题 / 标签过滤<br>• `--semantic*` 兼容 no-op | • Query 改写 / 意图识别<br>• 多轮编排 / 多次调用 CLI<br>• 证据合成 / 摘要 / 解释<br>• 引用整理 / 报告生成 |
...
| 分层人生检索                 | ✅   | CLI Core 离线完成分层检索：关键词精确匹配 + Entity Graph 实体扩展；Agent 可选负责搜索前编排和搜索后表达                                                    |
...
在稳固的 CLI Core 之上，Life Index 正在构建模组化的高级功能 —— 每个模组都是**稳定记录格式 + 结构化搜索 + Entity Graph 增强 + LLM 智能编排**的组合：
...
→ 全新 / 空数据安装的 pre-init `degraded` 属预期状态，简要说明即可，不当作失败；若 `safe_next_steps` 包含索引或 skill artifact 命令，按原顺序执行后重新运行 health；若 `safe_next_steps` 不包含这些命令，不要自行执行 rebuild / sync

**需要安装后 smoke test**
→ 只在临时 sandbox 中设置 `LIFE_INDEX_DATA_DIR` 后运行 `index`、`search --query ...`，并验证 `search --semantic --semantic-policy fallback` 可被接受但结果仍为关键词路径；不要向真实 `~/Documents/Life-Index/` 写入验证日志

**旧集成仍传 `--semantic*`**
→ 这是兼容 no-op，不会安装模型、构建向量索引或改变搜索结果。核心检索能力是关键词 + Entity Graph。

**venv 损坏（Python 升级后、迁移系统后）**
→ 先确认 `bootstrap --json` 已判定当前目录是目标安装目录；确认后再重建代码环境，且绝不触碰用户数据

**升级到新版本**
→ 先运行 onboarding bootstrap gate，并按返回的 `execution_policy` 与 `safe_next_steps` 执行；如果首项是 refresh 命令，先刷新再继续
