...
5. 先处理 `needs_human`，再按 `route` 和 `safe_next_steps` 顺序执行；如果 `safe_next_steps` 包含 `pip install -e .`，必须先重装再执行后续 migrate/health；不要把 "clean slate" / "fresh install" / "start from scratch" 理解为可以删除已有日志数据
...
**升级到新版本**
→ 先用平台 skill sync/resync 或 `git pull --ff-only` 刷新 checkout 代码，再用对应 venv 执行 `pip install -e .` 重装包，然后运行 onboarding Step 0 bootstrap gate，并按 `AGENT_ONBOARDING.md` 的 bootstrap / freshness / sync / repair 规则执行
