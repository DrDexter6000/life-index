...
QUERIES: list[tuple[str, str, str]] = [
    ...
    (
        "架构_决策",
        "最近对 Life Index 架构有哪些重大决策？",
        "应命中 CTO 评审 03-14_001、搜索 Round 系列等工作条目。",
    ),
    (
        "实体_模型",
        "我跟 Claude Opus 互动的记录有哪些？",
        "应命中 tags/body 含 Claude Opus 的条目，尤其 03-14_001。",
    ),
    ("天气_雨天", "3月份有哪几天下雨？", "应命中 weather 含 rain/雨 的 3 月条目。"),
    ("生日_计划", "生日相关的计划有哪些？", "应命中 03-11 计划回海边过生日。"),
    (
        "隐式时间",
        "上次生病或身体不舒服是什么时候？",
    ...
    ("项目_搜索", "Life Index 搜索系统最近做了哪些改进？", "应命中 Round 7/8/9 搜索相关条目。"),
    ("否定_测试", "我有没有提到过北京？", "若无命中应明确返回 0 结果（北京不是常驻地点）。"),
    ("模糊_情绪", "最近心情怎么样？", "应混合 mood 字段做聚合式召回。"),
]
...
def summarize(payload: dict) -> dict:
    ...
    sem_only = sum(
        1 for r in merged if (r.get("fts_score") or 0) == 0 and (r.get("semantic_score") or 0) > 0
    )
    fts_only = sum(
        1 for r in merged if (r.get("fts_score") or 0) > 0 and (r.get("semantic_score") or 0) == 0
    )
...
def main() -> int:
    ...
    for idx, (label, q, hint) in enumerate(QUERIES, start=1):
        ...
        print(
            f"  elapsed={r['elapsed']}s  l1={summary['l1_count']} "
            f"l2={summary['l2_count']} l3={summary['l3_count']} "
            f"sem={summary['semantic_count']} merged={summary['merged_count']} "
            f"mix={summary['mix']}"
        )
        for hit in summary["top"]:
            print(
                f"    - {hit.get('date')} | {hit.get('title')}  "
                f"fts={hit.get('fts')} sem={hit.get('sem')} rrf={hit.get('rrf')}"
            )
