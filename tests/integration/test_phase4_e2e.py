...
TEST_JOURNALS = [
    {"title": "想念尿片侠", "content": "看到女儿小时候的照片，感慨万分。小疙瘩长大了。", "date": "2026-03-01", "topic": "think"},
    {"title": "团团不认真吃饭", "content": "今天团团又不认真吃饭了，真是头疼。", "date": "2026-03-02", "topic": "life"},
    ...
    {"title": "想念我的女儿", "content": "想念团团，那个可爱的孩子。小疙瘩长大了。", "date": "2026-03-07", "topic": "life"},
...
class TestPhase4E2E:
    ...
    def test_semantic_query_finds_daughter(self, search_env):
        ...
        assert "团团" in top_title or "尿片侠" in top_title or "女儿" in top_title, (
    ...
    def test_rrf_score_positive_for_hybrid(self, search_env):
        ...
        result = search_env(query="团团", level=3)
    ...
    def test_final_score_geq_rrf_score(self, search_env):
        ...
        result = search_env(query="团团不认真吃饭", level=3)
        for r in result.get("merged_results", []):
