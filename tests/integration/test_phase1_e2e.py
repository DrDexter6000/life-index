...
class TestPhase1Integration:
    ...
    def test_full_phase1_chain(self, isolated_data_dir: Path) -> None:
        ...
        _create_journal(
            ...
            content="看到团团小时候的照片，那个只有2岁上下的尿片侠。",
        )
        _create_journal(
            month_dir,
        ...
        # "团团" appears 2x in people → should be added
        added_names = {e["primary_name"] for e in seed_result["added"]}
        assert "团团" in added_names, (
            f"Expected '团团' in added, got: {seed_result['added']}"
        )
        ...
        search_result = hierarchical_search("团团", semantic=False)
        ...
        # "团团" should be in skipped_existing
        skipped_names = {e["primary_name"] for e in seed_result_2["skipped_existing"]}
        assert "团团" in skipped_names, (
            f"'团团' should be skipped as existing, got: {seed_result_2['skipped_existing']}"
        )
    ...
    def test_write_candidates_with_existing_graph(
        ...
        # "团团" should match existing entity
        tuantuan = next((c for c in candidates if c["text"] == "团团"), None)
        assert tuantuan is not None, "Should find '团团' candidate"
