...
class TestSeedCandidateCollection:
    ...
    def test_extracts_people_as_person_type(self, isolated_data_dir: Path) -> None:
        ...
        # Create 2 journals mentioning "团团"
        _create_journal(isolated_data_dir, "日记1", people=["团团"])
        _create_journal(isolated_data_dir, "日记2", people=["团团", "妈妈"])
        ...
        tuantuan = next(c for c in candidates if c.primary_name == "团团")
    ...
    def test_frequency_threshold_filters_singletons(
        ...
        # "团团" appears once, "妈妈" appears twice
        _create_journal(isolated_data_dir, "日记1", people=["团团"])
        ...
        assert "团团" not in names, "Singleton should be filtered"
        assert "妈妈" in names
...
class TestSeedIdempotent:
    ...
    def test_second_run_no_changes(self, isolated_data_dir: Path) -> None:
        ...
        _create_journal(isolated_data_dir, "日记1", people=["团团", "妈妈"])
        _create_journal(isolated_data_dir, "日记2", people=["团团", "妈妈"])
    ...
    def test_preserves_existing_entity_aliases(self, isolated_data_dir: Path) -> None:
        """
        If graph already has {primary_name: "团团", aliases: ["尿片侠"]},
        ...
        # Pre-create graph with "团团" having alias "尿片侠"
        ...
        # "团团" should be in skipped_existing, not in added
        added_names = {e["primary_name"] for e in result["added"]}
        assert "团团" not in added_names

        # Verify aliases preserved
        entities = load_entity_graph(graph_path)
        tuantuan = next(e for e in entities if e["primary_name"] == "团团")
        assert tuantuan["aliases"] == ["尿片侠"], "Existing aliases must be preserved"
    ...
    def test_output_structure(self, isolated_data_dir: Path) -> None:
        ...
        graph_path = isolated_data_dir / "entity_graph.yaml"
        _create_journal(isolated_data_dir, "日记1", people=["团团"])
        _create_journal(isolated_data_dir, "日记2", people=["团团"])
