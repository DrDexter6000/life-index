...
class TestParseAndCacheJournalEdgeCases:
    ...
    def test_parse_and_cache_journal_preserves_string_topic_for_safe_upstream_normalization(
        ...
        """Legacy scalar topic values remain readable for upstream normalization."""
...
class TestCachePerformance:
    ...
    def test_cache_faster_than_parsing(self, tmp_path):
        """Test that cached reads avoid parsing and stay practically fast."""
        import time
        ...
        try:
            ...
            result1 = get_or_update_metadata(test_file, conn=conn)

            # Second call - must come from cache, not the parser.
            with patch(
                "tools.lib.metadata_cache.parse_and_cache_journal",
                side_effect=AssertionError("cache hit should not parse journal"),
            ):
                start = time.perf_counter()
                result2 = get_or_update_metadata(test_file, conn=conn)
        ...
        # Cache read should remain well below human-visible latency even on
        # slower Windows CI/storage.
        assert cache_time < 0.05, f"Cache read took {cache_time * 1000:.2f}ms, expected < 50ms"
