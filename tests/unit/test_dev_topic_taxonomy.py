...
class TestTopicTaxonomyNormalizer:
    def test_reports_non_standard_topics_with_suggested_standard_mapping(
        ...
        suggestions = {
            issue.original_topic: issue.suggested_topic for issue in result.issues
        }
        assert suggestions["learning"] == "learn"
        assert suggestions["general"] == "life"

    def test_apply_mode_rewrites_safe_deterministic_topic_mappings(
        self, tmp_path: Path
    ) -> None:
        from tools.dev.normalize_topic_taxonomy import TopicTaxonomyNormalizer
    ...
    def test_skips_unmapped_non_standard_topics_in_apply_mode(
        self, tmp_path: Path
    ) -> None:
        from tools.dev.normalize_topic_taxonomy import TopicTaxonomyNormalizer
    ...
    def test_report_includes_standard_topic_set_and_rebuild_hint(
        self, tmp_path: Path
    ) -> None:
        ...
        assert "work, learn, health, relation, think, create, life" in output
