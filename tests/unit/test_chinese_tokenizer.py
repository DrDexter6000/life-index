...
class TestEntityDictionary:
    ...
    def test_entity_name_not_split(self):
        """Entity name '团团' stays as single token after dict loading."""
        ...
        result = _tokens(segment_for_fts("团团哭了", mode="index"))
        assert "团团" in result

    def test_multi_char_entity_alias(self):
        """Multi-character alias like '小疙瘩' stays intact."""
        with TemporaryDirectory() as tmp:
            graph_path = _make_entity_graph(
                Path(tmp),
                [
        ...
        result = _tokens(segment_for_fts("想念尿片侠", mode="index"))
        assert "尿片侠" in result
