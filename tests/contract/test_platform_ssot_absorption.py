...
|---|---|
| Canonical journal and attachment mutation |
proposed / pending Human Owner substantive approval |
| Schema, validation, migration, transaction, locking, and audit |
proposed / pending Human Owner substantive approval |
| Deterministic indexing, retrieval, freshness, and evidence navigation |
proposed / pending Human Owner substantive approval |
...
|---|---|
| Core | Deterministic tools; no planning, reasoning, orchestration, interpretation,
or synthesis. |
| Host Agent + Skill | Owns planning, multi-hop reasoning, orchestration, interpretation,
and synthesis. |
| GUI | Presentation only; no intelligence; strict adapter stays GUI-owned. |
| Current bridge | Non-Core and GUI-owned. |
| Gateway | Optional future typed 1:1 projection under #164; unimplemented;
...
Current runtime: `--synthesize` requests provider-backed LLM synthesis and applies
the trust gate when its runtime prerequisites are available.

Target under #163: `--synthesize` becomes a deprecated no-op; this is not yet implemented.

Compatibility: retain the accepted flag for at least two major versions after the
#163 transition is implemented.
"""
...
- Core calls remain deterministic; Core does not plan, reason, orchestrate,
interpret, or synthesize.
- Gateway is an optional future typed 1:1 projection under #164; it is not yet
implemented, is not a second semantic API, and owns no intelligence. If introduced,
it is only a contract-equivalent transport. Gateway is never required for the core route.
"""
...
def _assert_named_block_snapshot(
    ...
    ), f"{source} must contain exactly one {marker} block"
    start = text.index(start_marker) + len(start_marker)
    end = text.index(end_marker, start)
    block = text[start:end]
    actual_normalized = " ".join(block.split())
    expected_normalized = " ".join(expected.split())
    if actual_normalized != expected_normalized:
        diff = "\n".join(
            difflib.unified_diff(
                textwrap.wrap(expected_normalized, width=100),
                textwrap.wrap(actual_normalized, width=100),
...
def test_role_contract_rejects_appended_contradictions_and_external_duty_verbs() -> None:
    ...
    no_orchestration = skill.replace("They also own orchestration.", "", 1)
    with pytest.raises(AssertionError):
        _assert_named_block_snapshot(
            no_orchestration,
            "HOST-AGENT-ROUTING",
...
def test_deprecation_text_names_owner_issue_and_runtime_state() -> None:
    api = _read_authority("api")
    skill = _read_authority("skill")
    _assert_named_block_snapshot(
        api,
        "SYNTHESIZE-TRANSITION",
        EXPECTED_API_SYNTHESIZE_TRANSITION,
